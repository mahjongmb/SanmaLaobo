...
function shouldCpuDiscardCandidateRiichi(snapshot, candidate, profile){
  ...
  const isWideRyanmenRoute = tenpai.isRyanmenWait && tenpai.waitTileCount >= 5;
  const isMultiGoodRoute = tenpai.waitTypeCount >= 2 && tenpai.waitTileCount >= 4;
  const isGoodWaitRoute = isRyanmenGoodRoute || isWideRyanmenRoute || isMultiGoodRoute || tenpai.waitTileCount >= 4;
  const isExcellentWaitRoute = tenpai.isExcellentWait || isWideRyanmenRoute || (tenpai.waitTypeCount >= 3 && tenpai.waitTileCount >= 5);
  ...
  if (isNoYakuHand && tenpai.isBadWait && !isWideRyanmen && !(context.isLast && tenpai.waitTileCount >= 3)) return false;
  ...
  if (hasDamaReason && context.threatCount <= 0 && !context.isLast && !isHighValueDama && tenpai.waitTileCount <= 4){
    return false;
  }

  // --- 肯定条件（オーラス救済を縮小・高打点愚形を解禁）---
  if (tenpai.isExcellentWait && (!hasDamaReason || damaValue.score < 1.45 || context.isLast)) return true;
  ...
  if (context.isDealer && isStrongRyanmen && damaValue.score < 1.5) return true;
  if (isWideRyanmen && context.threatCount <= 1 && damaValue.score < 1.5) return true;
  if (isStrongRyanmen && context.threatCount <= 0 && context.phase !== "end" && damaValue.score < 1.4) return true;
  if (tenpai.waitTileCount >= 7 && damaValue.score < 1.6) return true;
